 ...
 * 
 */
public class SimplePatternMatcherTest extends TestCase {
	...
	assertTrue("null pattern should return an empty collection", result
		.isEmpty());

	mpattern = "";
	result = SimplePatternMatcher.parseMultiPatternString(mpattern, ";");
	assertTrue("empty pattern should return an empty collection", result
		.isEmpty());

	mpattern = "   \n\t   ";
	result = SimplePatternMatcher.parseMultiPatternString(mpattern, ";");
	assertTrue("trimmed empty pattern should return an empty collection",
		result.isEmpty());
	...
	mpattern = "/faces/*;;*.jsf";
	result = SimplePatternMatcher.parseMultiPatternString(mpattern, ";");
	assertTrue("'" + mpattern + "' should return an collection of size==2",
		result.size() == 2);
    }
	...
	source = "";
	expected = source;
	assertEquals(SimplePatternMatcher.regexify(source), expected);
    ...
    private static final class MatchTest extends TestCase {
	...
	void test() {
		...
		assertTrue(match + " should match " + this.matcher,
			this.matcher.matches(match));
	    }
	    for (String dontMatch : nonMatching) {
		assertFalse(dontMatch + " should not match " + this.matcher,
